            ...
            <form method="POST" action="{{ route('fiyat.hesapla') }}">
                ...
                <div class="row mb-3">
                        ...
                        <select class="form-select" id="ulke" name="ulke" required>
                            ...
                            @foreach ($ulkes as $ulkem)
                                <option value="{{ $ulkem->ad }}"
                                    {{ old('ulke', $ulke ?? '') == $ulkem->ad ? 'selected' : '' }}>{{ $ulkem->ad }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.1" min="0.1" class="form-control" id="agirlik"
                            name="agirlik" value="{{ old('agirlik', $agirlik ?? '') }}" required>
                    </div>
                </div>

                <div class="form-check mb-3">
                    <input class="form-check-input" type="checkbox" id="olcuSec" name="olcuSec"
                ...
                <div class="row mb-3" id="olcuAlanlari">
                    <div class="col">
                        ...
                        <input type="number" step="0.1" class="form-control" id="en" name="en"
                            value="{{ old('en', $en ?? '') }}">
                    </div>
                    <div class="col">
                        <label for="boy" class="form-label">Boy (cm)</label>
                        <input type="number" step="0.1" class="form-control" id="boy" name="boy"
                            value="{{ old('boy', $boy ?? '') }}">
                    </div>
                    <div class="col">
                        <label for="yukseklik" class="form-label">Yükseklik (cm)</label>
                        <input type="number" step="0.1" class="form-control" id="yukseklik" name="yukseklik"
        ...
        @if (isset($fiyatlar) && count($fiyatlar) > 0)
            <div class="mb-3">
                ...
                <div class="alert alert-warning small">
                    Gördüğünüz fiyatlar başlangıç fiyatlarıdır. Gönderinizin ölçüleri, paket tipi veya varış bölgesine göre
                    değişebilir.
                </div>
            </div>
            ...
            <div class="card p-3">
                ...
                @foreach ($fiyatlar as $fiyat)
                    <div class="row align-items-center mb-3">
                        <div class="col-md-2">
                            <span class="badge bg-warning col-lg-12  p-2"
                                style="font-size:14px; font-weight: 900;">{{ $fiyat->tasiyici }}</span>

                        </div>
                        <div class="col-md-3">
                            <span class="badge bg-primary col-lg-6  p-2">{{ $fiyat->hizmet_tipi }}</span>
                            @if ($loop->first)
...
@push('scripts')
    <script>
        ...
        window.addEventListener('load', () => {
            document.getElementById('olcuAlanlari').style.display = document.getElementById('olcuSec').checked ?
                'flex' : 'none';
        });
    </script>
@endpush
